...
const lateClear = (domCopies = [], scope: any) => {
    domCopies.forEach((dom: any) => {
 ...
 * @param {Function[]} arr
 */
const runAll = (arr: any) => arr.forEach((f: any) => f());
 ...
 */
const nodePool = (template: string) => ({
    ...
    alloc(amount: number) {
        ...
        if (diff > 0) {
            ...
            this.pool = this.pool.concat(/** @type {HTMLElement[]} */ Array.from(tpl.content.children));
        }
    },
    /**
     * @param {number} amount
...
const repeatDirective = {
    ...
    process: ({targetNode: tNode, scopeNode: scope, expression: ex}: any) => {
        const repeatCleanup = parseInt(tNode.getAttribute(REPEAT_CLEANUP)) || 5000;
        tNode[block] = 'abort';
        ...
        function update(/** @type {any[]} */ dataSource = [], /** @type {boolean} */ forceUpdate = false) {
            if (cleanupInterval) {
